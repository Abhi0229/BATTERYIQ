...
import { Zap, Activity, Battery, ShieldAlert, Cpu, History, FileText, MapPin, BarChart3, Upload, ChevronRight, Lightbulb } from 'lucide-react';
...
function DriverView() {
  ...
  const getTips = (data: any) => {
    const tips = [];
    if (data.health_score < 75) tips.push({ icon: "🔧", text: "Health below 75%. Schedule a deep diagnostic scan at a certified center." });
    else if (data.health_score < 90) tips.push({ icon: "📈", text: "Degradation is within bounds. Avoid 100% SoC deep-cycles to preserve health." });
    else tips.push({ icon: "✅", text: "Excellent health profile. Continue your current charging and driving patterns." });

    if (data.stress_label === 'High') tips.push({ icon: "🌡️", text: "Thermal stress detected. Reducing drive intensity can lower internal resistance heat." });
    else if (data.stress_label === 'Medium') tips.push({ icon: "⚡", text: "Moderate stress. Balanced acceleration helped maintain this profile." });
    
    if (data.RUL < 50) tips.push({ icon: "⚠️", text: `Critical Cycle Threshold: Only ${data.RUL} sessions remaining until suggested overhaul.` });
    else tips.push({ icon: "🔋", text: `High longevity potential. Expected to last ${data.RUL} more cycles at current usage.` });

    if (data.efficiency_score < 80) tips.push({ icon: "🌀", text: "Efficiency loss detected. Check for parasitic loads or tire pressure issues." });
      ...
      {activeTab === 'overview' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-12 xl:col-span-5 space-y-6">
            ...
            <div className="glass-card p-6 flex items-center gap-6 group">
              <div className="relative w-16 h-28 border-2 border-muted-foreground/30 rounded-lg p-1 transition-all group-hover:border-primary/50">
                <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-6 h-2 bg-muted-foreground/30 rounded-t-sm" />
                <div 
                  className="w-full transition-all duration-700 rounded-sm shadow-[0_0_15px_rgba(0,0,0,0.2)]" 
                  style={{ 
                    height: `${params.battery_pct}%`, 
                    marginTop: `${100 - params.battery_pct}%`,
                    backgroundColor: params.battery_pct < 20 ? '#ef4444' : params.battery_pct < 50 ? '#f59e0b' : '#00d97e',
                    boxShadow: `0 0 20px ${params.battery_pct < 20 ? 'rgba(239,68,68,0.4)' : params.battery_pct < 50 ? 'rgba(245,158,11,0.4)' : 'rgba(0,217,126,0.4)'}`
                  }} 
                />
              </div>
              <div className="flex-1">
                <h4 className="font-bold mb-1 text-primary flex items-center gap-2">
                  <Battery className="w-4 h-4" /> Battery Viz
                </h4>
                <p className="text-xs text-muted-foreground leading-relaxed">Visualizing real-time SOC simulation based on sensor telemetry.</p>
              </div>
            </div>
          </div>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4 gap-4">
                  <CircularGauge 
                    ...
                    sublabel={results.health_info.status} 
                    color={results.health_info.color}
                    unit="%"
                  />
                  <CircularGauge 
                    label="Stress Level" 
                    value={results.stress_label === 'Low' ? 20 : results.stress_label === 'Medium' ? 55 : 90} 
                    sublabel={results.stress_info.message} 
                    color={results.stress_info.color}
                    customValue={results.stress_label}
                  />
                  <CircularGauge 
                    label="Efficiency" 
                    value={Math.round(results.efficiency_score || 0)} 
                    sublabel="Discharge Efficiency" 
                    color={results.efficiency_score > 85 ? 'success' : 'primary'}
                    unit="%"
                  />
                  <CircularGauge 
                    label="Cycles Left" 
                    value={Math.min(100, (results.RUL / 500) * 100)} 
                    sublabel={`${results.RUL} cycles`} 
                    color={results.rul_info.color}
                    unit=""
                    customValue={results.RUL}
                  />
                </div>
                ...
                <div className="glass-strong rounded-2xl p-6 relative overflow-hidden group">
                  ...
                  <h4 className="font-bold flex items-center gap-2 mb-4">
                    <Lightbulb className="w-5 h-5 text-yellow-500" /> Strategic Reasoning (Why this result?)
                  </h4>
                  <div className="p-4 rounded-xl bg-white/5 border border-white/5 mb-6">
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      {results.stress_label === 'High' 
                        ? `Your battery is experiencing high stress primarily due to the current temperature of ${params.temperature}°C in combination with a ${results.health_score}% health score. High thermal loads at this stage of the lifecycle accelerate chemical depletion.`
                        : results.health_score < 80 
                        ? `The health degradation to ${results.health_score}% is a direct correlation of reaching ${params.cycle_count} charge cycles. At this phase, internal resistance begins to climb, affecting discharge efficiency.`
                        : "Your system profile is optimized. The balanced load and standard temperature range are preserving the electrode integrity, resulting in a low stress profile."
                      }
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {getTips(results).map((tip, i) => (
                      <div key={i} className="flex gap-3 p-3 rounded-lg bg-white/10 border border-white/10 items-start hover:bg-white/15 transition-colors">
                        <span className="text-lg">{tip.icon}</span>
                        <span className="text-[11px] leading-relaxed text-foreground/90 font-medium">{tip.text}</span>
                      </div>
                    ))}
                </div>
              </div>
        ...
        <div className="glass-strong rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <a href="/api/export_pdf" target="_blank" className="text-xs text-primary hover:underline font-bold flex items-center gap-1">
          ...
          <tbody className="divide-y divide-white/5">
            {history.map((h) => (
              <tr key={h.id} className="hover:bg-white/5 transition-colors group">
                ...
                <td className="p-4">
                  <a href={`/api/export_pdf/${h.id}`} target="_blank" className="p-2 hover:bg-white/10 rounded-lg text-primary transition-all inline-block">
...
function CircularGauge({ value, label, sublabel, color, unit = "", customValue }: any) {
  const data = [{ value }];
  const cMap: any = {
    success: '#10b981',
    primary: '#3b82f6',
  ...
  const activeColor = cMap[color] || cMap.primary;

  return (
    <div className="glass-card p-5 flex flex-col items-center justify-center text-center group">
      <div className="relative w-40 h-40 mb-2">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl font-bold text-foreground">{customValue !== undefined ? customValue : value}{unit}</span>
        </div>
      </div>
      <p className="text-xs text-muted-foreground uppercase font-bold mb-1">{label}</p>
      <p className="text-[10px] opacity-80 font-medium" style={{ color: activeColor }}>{sublabel}</p>
    </div>
  );
}
