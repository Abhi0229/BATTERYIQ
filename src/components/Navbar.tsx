...
const servicesDropdown: DropdownItem[] = [
  ...
  { label: 'ROI Calculator', href: '/roi-calculator', icon: <DollarSign className="w-4 h-4" />, desc: 'Calculate fleet savings' },
];
        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navItems.map((item) => (
              ...
              >
                <Link 
                  to={item.key === 'company' ? '/about' : `/${item.key}`}
                  className={`flex items-center gap-1 px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 ${activeDropdown === item.key ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}
                >
                  {item.label}
